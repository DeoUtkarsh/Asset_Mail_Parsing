...
> **Branch:** `feature/frontend-redesign` — local / feature development.  
> `AUTO_FETCH_IMAP_IDLE` defaults **true** here so you can test live inbox progress locally.  
> Set it `false` in `.env` if you only want manual Fetch. **AWS deploys from `aws-deployment`.**
...
Runs **locally** against PostgreSQL. Production/dev CloudFront deploy uses the sibling
**`aws-deployment`** branch — see that branch’s README and `docs/aws-deployment/`.
...
```
Gmail (IMAP)
   │  ⬇ Fetch Emails  (and/or local IMAP IDLE when AUTO_FETCH_IMAP_IDLE=true)
...
# This branch defaults AUTO_FETCH_IMAP_IDLE=true — set false to disable live IDLE
...
1. **Fetch Emails** — IMAP pull of **new** broker mail (optional date range); Phase 1 extraction + contacts; SSE progress. Sync toast when Phase 1 completes. With IDLE on, new INBOX mail can also auto-run the same Phase-1 path.
...
**Do not deploy this branch to ECS by default.** Use **`aws-deployment`**.

Live **dev** app (from aws-deployment): **https://d2bt5vx8sl8jq9.cloudfront.net**  
Runbook: **[docs/aws-deployment/README.md](docs/aws-deployment/README.md)**
...
├── docs/aws-deployment/
├── backend/
│   ├── .env.example               ← AUTO_FETCH_IMAP_IDLE=true on this branch
│   ├── requirements.txt
│   ├── main.py                    ← Phase-1 → enrichment hook
│   ├── vessel_enrichment.py       ← Claude web_search library enrichment
│   ├── imap_idle_watcher.py / imap_uid_watermark.py / fetch_scope.py
│   ├── scripts/reset_data.py / autofill_vessel_library.py
│   └── agents/ …
└── frontend/
    └── src/
...
| `AUTO_FETCH_IMAP_IDLE` | **`true` by default on this branch** (local IDLE testing) |
| `AUTO_FETCH_IDLE_RECONNECT_SEC` | IDLE reconnect pause (default `30`) |
| `ATTACHMENTS_S3_BUCKET` | Empty = local `attachment_files/` |
