...
/** Contact List columns — company/address live in accordion headers, not as columns. */
...
/** Pinned on horizontal scroll — PIC name only. */
export const PINNED_CONTACT_KEYS = ["contact_name"];

/** Default data columns (no company, address, Sr. No, or attachment). */
export const DEFAULT_CONTACT_COLUMNS = [
  ...
  { key: "mob_phone", label: "MOBILE", minW: 120, fieldKey: "mob_phone" },
  { key: "vessel_name", label: "VESSEL NAME", minW: 180, fieldKey: "vessel_name" },
...
/** Extra fields when All columns is on. */
export const EXTRA_CONTACT_COLUMNS = [
  ...
  { key: "country", label: "COUNTRY", minW: 100, fieldKey: "country" },
...
export function buildContactSerialMap(rows, buildGroups) {
  const groups = typeof buildGroups === "function" ? buildGroups(rows) : [];
  const map = new Map();
  let n = 0;
  const sections = Array.isArray(groups) && groups[0]?.groups
    ? groups
    : [{ groups }];
...
/** Split multi-vessel cells (semicolon / comma / pipe / newline). */
export function splitVesselNames(value) {
  return String(value || "")
    .split(/[;|,\n]+/)
    .map((s) => s.trim())
    .filter(Boolean);
