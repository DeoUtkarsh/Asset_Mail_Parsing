...
| `AUTO_FETCH_IMAP_IDLE` | **`true` on ECS** and on `feature/frontend-redesign` local default — IMAP IDLE auto-runs Phase 1 when new mail arrives. Set `false` in `.env` to disable |
